				...
				$this.animate({
					...
					complete: function () {
						...
						if (speed > 0.5) {
							...
							$this.stop();
							var itemHeight = $this.children().first().outerHeight();
							var visibleCount = Math.round($this.parent().height() / itemHeight);
							var middleIndex = Math.floor(visibleCount / 2);
							var $centeredItem = $this.children().eq(middleIndex % $this.children().length);
							settings.done(`${$centeredItem.html()}, ${itemHeight}, ${visibleCount}, ${middleIndex}`);
